...
The distributed API produces those jobs and consumes their result/event stream, but the bridge has not been validated by a real Browserless end-to-end login because no upstream credentials are available and the Browserless image pull may be unavailable. The distributed API therefore does not claim full browser-automation feature parity. `AUTOMATION_EXECUTOR_READY` remains an explicit opt-in gate, and inline Google browser automation is rejected with an explicit backend-unconfigured error until the bridge is enabled. Patchright remains permitted only for local/single-worker execution; distributed API and worker configuration reject it.

Live PostgreSQL and Redis HA tests pass, including two-process coordination and outage/drain checks. Those results validate the shared self-hosted HA foundation, not Browserless login coverage or Kubernetes readiness.

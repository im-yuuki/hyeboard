...
The worker needs Redis, Browserless/Puppeteer, the current automation key pair, and its stream/health configuration. Prefer the repository `docker-compose.yml` `distributed` profile for local container orchestration; it supplies the private network endpoints and health checks. Kubernetes uses the same image through `deploy/k8s/base` and its `example`, `staging`, and `production` overlays, while PostgreSQL, Redis, Browserless, TLS, and secrets remain external to the cluster templates.

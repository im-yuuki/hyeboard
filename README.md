...
The encrypted automation protocol, Redis Streams worker, leases/fencing, reclaim, cancellation, graceful drain, Browserless provider, distributed API enqueue/event/CAPTCHA protocol, and host CLI bridge are implemented. The bridge owns the Puppeteer session and supplies it to the UET adapter through `AutomationExecutor`; `AUTOMATION_EXECUTOR_READY` remains an explicit deployment gate. Live PostgreSQL and Redis HA tests pass, including two-process coordination and outage/drain checks. A real Browserless end-to-end login has not run because no upstream credentials are available and the Browserless image pull may be unavailable. Do not treat this as full feature parity or Kubernetes readiness; Kubernetes manifests remain deferred until the remaining multi-replica and failure-injection gates pass. Details and opt-in Testcontainers commands are in [docs/ha-runbook.md](docs/ha-runbook.md).

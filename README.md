...
## Runtime status and deferred deployment work
...
The encrypted automation protocol, Redis Streams worker, leases/fencing, reclaim, cancellation, graceful drain, Browserless provider, distributed API enqueue/event/CAPTCHA protocol, and host CLI bridge are implemented. The bridge owns the Puppeteer session and supplies it to the UET adapter through `AutomationExecutor`; `AUTOMATION_EXECUTOR_READY` remains an explicit deployment gate. The Browserless image is pinned to `ghcr.io/browserless/chromium:v2.55.4`, was pulled manually, and started successfully; a live Puppeteer CDP smoke test passed against `ws://127.0.0.1:3000/chromium`, including a token query. A real distributed Browserless/UET Google login was attempted with credentials supplied through the local ignored `.env`, using local PostgreSQL/Redis, the API, and the automation worker. `/api/ready` reached ready, and `pnpm test:ha` passed PostgreSQL 5/5 and Redis 4/4. The login reached queue/login progress `0, 10, 35, 35, 60` before returning HTTP 502 with code `GOOGLE_SIGNIN_FAILURE`; the worker logged Puppeteer `Attempted to use detached Frame ...` while waiting for Keycloak `#username`. This real login did not pass, so do not treat the result as full feature parity or Kubernetes readiness; Kubernetes manifests remain deferred until the remaining multi-replica and failure-injection gates pass. The node-redis stream read-shape fix was committed as `f7c78fd`. Details and opt-in Testcontainers commands are in [docs/ha-runbook.md](docs/ha-runbook.md).

...
if (!infrastructureAvailable) {
  console.warn(`[ha:redis] SKIPPED: ${dockerAvailable ? "required Docker images are not present (pull postgres:16-alpine and redis:7-alpine first)" : "Docker is unavailable"}; no fake integration result will be reported.`);
}

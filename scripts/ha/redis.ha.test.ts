...
const imagesAvailable =
  dockerAvailable &&
  (await dockerImagesAreAvailable(["postgres:16-alpine", "redis:7-alpine"]));
...
if (!infrastructureAvailable) {
  ...
  console.warn(
    `[ha:redis] SKIPPED: ${reason}; no fake integration result will be reported.`,
  );
}
...
describe.skipIf(!infrastructureAvailable)("HA Redis integration", () => {
  ...
  beforeAll(async () => {
    ...
    if (
      containers[0]?.status === "rejected" ||
      containers[1]?.status === "rejected"
    ) {
      await Promise.allSettled(
        containers.map((result) =>
          result.status === "fulfilled" ? result.value.stop() : undefined,
        ),
      );
      throw containers.find(
    ...
    workers = startedWorkers.flatMap((result) =>
      result.status === "fulfilled" ? [result.value] : [],
    );
    const workerFailure = startedWorkers.find(
  ...
  it("serializes refresh leases across independent Redis processes", async () => {
    ...
    const pair = {
      accessTokenId: "C".repeat(22),
      accessExpiresAt: Date.now() + 600_000,
      grantId: "D".repeat(22),
      grantExpiresAt: Date.now() + 900_000,
    };
    await runProbe(
      "redis",
      "activate",
  ...
  it("reclaims a crashed automation consumer with a new fencing lease", async () => {
    ...
    try {
      ...
      const messageId = await firstBroker.add(stream, {
        jobEnvelope: "opaque",
      });
      await expect(
      ...
      expect(reclaimed).toMatchObject([{ id: messageId }]);
      expect(reclaimed[0]?.deliveryCount).toBeGreaterThanOrEqual(1);
  ...
  it("relays a CAPTCHA answer between independent Redis processes", async () => {
    const prepared = await runProbe(
      "redis",
      "captcha-prepare",
      { image: "synthetic-image-not-persisted-in-result" },
      { redisUrl: redis.getConnectionUrl() },
    );
    const waiting = runProbe(
      "redis",
      "captcha-wait",
    ...
    await expect(
      runProbe(
        "redis",
        "captcha-answer",
        { challengeId: prepared.challengeId, answer: "CROSS_PROCESS_OK" },
        { redisUrl: redis.getConnectionUrl() },
      ),
    ).resolves.toEqual({ accepted: true });
    await expect(waiting).resolves.toMatchObject({
      answer: "CROSS_PROCESS_OK",
    });
  });

  it("keeps liveness independent while Redis outage is reported by readiness", async () => {
    await redis.stop();
    await expect(workers[0]!.request("/api/ready")).resolves.toMatchObject({
      status: 200,
      body: { data: { state: "ready" } },
    });
    await expect(
      workers[0]!.request("/api/uet/auth/import-session", {
        method: "POST",
      ...
      status: 503,
      body: { error: { code: "HA_DEPENDENCY_UNAVAILABLE" } },
    });
    await expect(workers[0]!.request("/api/live")).resolves.toMatchObject({
      status: 200,
  ...
  it("drains and exits both workers on SIGTERM", async () => {
    const results = await Promise.all(
      workers.map((worker) => worker.stop("SIGTERM")),
    );
    expect(
